...
def parse_args():
    parser = argparse.ArgumentParser(description='Static Intent Classification with Pre-renamed Dataset')
    ...
    parser.add_argument('--train_file', type=str, required=True,
                        help='Path to pre-renamed training file')
...
def load_intent_mappings(mapping_file: str) -> Tuple[dict, dict]:
    """Load intent name mappings"""
    mapping_df = pd.read_csv(mapping_file)
    forward_mapping = dict(zip(mapping_df['original_intent'], mapping_df['renamed_intent']))
    reverse_mapping = dict(zip(mapping_df['renamed_intent'], mapping_df['original_intent']))
...
def main():
    ...
    forward_mapping, reverse_mapping = load_intent_mappings(args.mapping_file)
    ...
    for idx, row in tqdm(test_df.iterrows(), total=total, desc="Processing queries"):
        ...
        examples, selected_intents, is_true_label_in_examples = retrieve_examples(
            query, mapped_true_label, train_df, retriever, args.total_examples)
        ...
        result = {
            ...
            'mapped_true_label': mapped_true_label,
            'predicted_intent': predicted_intent,
            'original_predicted_intent': original_predicted_intent,
