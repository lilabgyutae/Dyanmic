...
def parse_args():
    parser = argparse.ArgumentParser(description='Dynamic Intent Classification (Llama Mapping + Qwen Classification)')
    parser.add_argument('--retriever_model', type=str, default='all-mpnet-base-v2',
                        help='Sentence transformer model for retrieval')
    parser.add_argument('--mapping_model_id', type=str, default='model',
                        help='Model ID for intent mapping/renaming (Llama)')
    parser.add_argument('--classification_model_id', type=str, default='model',
                        help='Model ID for final intent classification (Qwen)')
...
def prompt_template(query: str, examples: List[Tuple[str, str, float]]) -> str:
    # mapped_intent를 사용하도록 변경
    intent_examples_str = "\n".join([f'Text: "{sent}"\nIntent: {intent}\n' 
                                   for sent, intent, _ in examples])
...
def main():
    ...
    # relabeling을 위한 Llama 모델 로드
    print("Loading mapping model (Llama)...")
    mapping_model, mapping_tokenizer = load_model(args.mapping_model_id, device)
    
    # 분류를 위한 Qwen 모델 로드
    print("Loading classification model (Qwen)...")
    classification_model, classification_tokenizer = load_model(args.classification_model_id, device)
    ...
    for idx, row in tqdm(enumerate(test_df.iterrows(), 1), total=total, desc="Processing queries"):
        ...
        examples = retrieve_similar_examples(query, train_df, retriever, args.total_examples)
        true_label_in_examples = any(intent == true_label for _, _, intent in examples)
        
        intent_mapping = rename_intents(examples, mapping_model, mapping_tokenizer, device)
        ...
        response = classification_tokenizer.decode(outputs[0], skip_special_tokens=True)
        ...
        is_correct = predicted_original_intent.lower() == true_label.lower()
        if is_correct:
            correct += 1
